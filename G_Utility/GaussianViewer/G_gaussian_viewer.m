...
% ---- Modes tab ----------------------------------------------------------
% ColumnSortable's click-to-sort was tried (both a live and a cached read
% of DisplayData to map the clicked row back to a mode index) and found
% unreliable in real interactive use -- sorting by Raman intensity and
% clicking a row rendered the wrong mode. Replaced with an explicit
% "Order by:" dropdown, the same proven mechanism G16_modeViewer already
% uses for its own mode list: it physically reorders modesTable.Data
% (column 1 always the true mode index), so there is no separate
% sorted-view state to get out of sync with -- onModeSelected can just
% read Data{row,1} directly.
uilabel(tabModes, 'Position', [5 486 55 18], 'Text', 'Order by:');
orderDD = uidropdown(tabModes, 'Position', [62 484 sidebarW-97 22], ...
    'Items', {'Mode number','Frequency','IR intensity','Raman intensity'}, ...
    'Value', 'Mode number', 'ValueChangedFcn', @(s,e) updateModesTable());
modesTable = uitable(tabModes, 'Position', [5 45 sidebarW-30 434], ...
...
uilabel(tabMO, 'Position', [5 418 200 16], 'Text', 'MO index (HOMO/LUMO/HOMO-n/n):');
...
isoValMOField = uieditfield(tabMO, 'numeric', 'Position', [120 362 sidebarW-150 22], 'Value', 0.02);
...
uilabel(tabMO, 'Position', [5 190 110 18], 'Text', 'IsoValue:');
isoValDensityField = uieditfield(tabMO, 'numeric', 'Position', [120 188 sidebarW-150 22], 'Value', 0.001);
spinDensityCheck = uicheckbox(tabMO, 'Position', [5 162 sidebarW-30 22], ...
    'Text', 'Spin density (UHF/UKS only)', 'Value', false);
uibutton(tabMO, 'push', 'Position', [5 130 sidebarW-30 28], ...
    'Text', 'Render Density', 'FontWeight', 'bold', 'ButtonPushedFcn', @(s,e) onRenderDensity());

uilabel(tabMO, 'Position', [5 96 sidebarW-30 18], 'Text', 'Electrostatic Potential', 'FontWeight', 'bold');
uilabel(tabMO, 'Position', [5 72 110 18], 'Text', 'IsoValue:');
isoValESPField = uieditfield(tabMO, 'numeric', 'Position', [120 70 sidebarW-150 22], 'Value', 0.001);
uilabel(tabMO, 'Position', [5 38 sidebarW-30 30], 'WordWrap', 'on', 'FontColor', [0.55 0.35 0.05], ...
    'Text', 'Can be slow (seconds to hours). See the command window for progress.');
uibutton(tabMO, 'push', 'Position', [5 4 sidebarW-30 28], ...
...
% -------------------------------------------------------------------------
    function updateModesTable()
        ...
        switch orderDD.Value
            case 'Frequency'
                [~, order] = sort(nm.freq, 'descend');
            case 'IR intensity'
                [~, order] = sort(nm.IR, 'descend');
            case 'Raman intensity'
                [~, order] = sort(nm.Raman, 'descend');
...
% -------------------------------------------------------------------------
    function onModeSelected(evt)
        ...
        row = evt.Indices(1,1);
        % Ordering is applied by physically permuting modesTable.Data
        % (see updateModesTable/orderDD) rather than via uitable's
        % built-in ColumnSortable click-to-sort, which was tried twice
        % (reading a live and then a cached DisplayData) and found
        % unreliable in real use -- clicking a row after sorting by Raman
        % intensity rendered an unrelated mode. Since Data itself always
        % matches what's displayed now, reading it directly is safe.
    ...
    function onRenderMO()
        ...
        try
            G_draw_mo_surface(fchkData, resolveMoField(), 'Ax', ax, ...
                'IsoValue', isoValMOField.Value, 'GridSpacing', gridSpField.Value, ...
...
% -------------------------------------------------------------------------
    function onRenderDensity()
        ...
        try
            G_draw_density_surface(fchkData, 'Ax', ax, 'IsoValue', isoValDensityField.Value, ...
                'GridSpacing', gridSpField.Value, ...
                'Padding', paddingField.Value, 'ShowMolecule', showMolCheck.Value, ...
...
% -------------------------------------------------------------------------
    function onRenderESP()
        ...
        d = uiprogressdlg(fig, 'Title', 'Rendering ESP surface...', ...
            ...
            'Indeterminate', 'on');
        try
            G_draw_esp_surface(fchkData, 'Ax', ax, 'IsoValue', isoValESPField.Value, ...
                'GridSpacing', gridSpField.Value, ...
                'Padding', paddingField.Value, 'ShowMolecule', showMolCheck.Value, ...
                'ShowLabels', showLabelsCheck.Value);
            applyLabelFontSize();
