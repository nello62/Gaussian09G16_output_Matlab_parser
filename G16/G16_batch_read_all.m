function T = G16_batch_read_all(folder, varargin)
% G16_BATCH_READ_ALL  Runs G16_READ_ALL over every Gaussian 16 output file
%                     in a folder and aggregates the key results into one
%                     summary table.
%
%   T = G16_BATCH_READ_ALL(folder)
%   T = G16_BATCH_READ_ALL(folder, 'Recursive', true)
%   T = G16_BATCH_READ_ALL(folder, 'WriteReports', true)
%   T = G16_BATCH_READ_ALL(folder, 'SaveAs', 'summary.xlsx')
%
...
%   an incomplete/crashed job, or a Gaussian 09 file run through the G16
...
%                        G16_WRITE_REPORT .txt next to each source file
...
%
%   See also G16_READ_ALL, G16_WRITE_REPORT, G16_ORBITAL_ENERGIES.
...
if ~isfolder(folder)
    error('G16_batch_read_all: folder not found: %s', folder);
end
...
if isempty(d)
    error('G16_batch_read_all: no .log/.out files found in %s', folder);
end
...
for k = 1:Nfiles
    ...
    try
        ...
        if write_reports
            ...
            G16_write_report(Tk, fullfile(fdir, [fname, '_report.txt']));
        end
    catch err
