function outfile = G16_animate_mode(mol, nm, mode_idx, varargin)
% G16_ANIMATE_MODE  Exports an MP4 animation of a vibrational mode.
%
%   G16_ANIMATE_MODE(mol, nm, mode_idx) oscillates the molecule along the
...
%   MPEG-4 profile).
%
%   OUTFILE = G16_ANIMATE_MODE(...) returns the path written to.
%
...
%       'AtomScale'       - see G16_draw_molecule (default 0.35)
%       'BondTol'         - see G16_draw_molecule (default 1.30)
%       'ShowLabels'      - see G16_draw_molecule (default false)
...
%   avoid interacting with it until G16_ANIMATE_MODE returns.
%
%   Example:
...
%
%   See also G16_DRAW_MODE, G16_DRAW_MOLECULE, G16_MODEVIEWER.
...
if mol.Natoms ~= nm.Natoms
    error('G16_animate_mode:sizeMismatch', ...
...
% -------------------------------------------------------------------------
% Displacement amplitude (mirrors G16_draw_mode's normalisation)
% -------------------------------------------------------------------------
U = nm.disp(:, :, mode_idx);
if flip_sign
    U = -U;
...
if max_norm == 0
    error('G16_animate_mode:zeroDisplacement', ...
...
% not jitter frame to frame (G16_draw_molecule's own 'axis tight' would
...
% across the BondTol threshold (G16_draw_molecule's default distance-based
...
fprintf('G16_animate_mode: animation saved to %s (%d frames, %.1f fps)\n', ...
