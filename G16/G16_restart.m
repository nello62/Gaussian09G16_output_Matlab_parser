function gjf_file = G16_restart(filename, varargin)
% G16_RESTART  Generates a Gaussian 09/16 input file (.gjf) for restarting
%              a calculation from a geometry in an existing Gaussian 16
%              .log/.out file.
%
...
if ~isfile(filename)
    error('G16_restart: file not found: %s', filename);
end
...
if isempty(route_sep_start) || isempty(route_sep_end)
    error('G16_restart: route section not found in %s', filename);
end
...
% ── Geometry ─────────────────────────────────────────────────────────────────
mol = G16_structure(filename, 'step', step_req, 'Lines', lines);
...
if fid < 0
    error('G16_restart: cannot create output file: %s', out_file);
end

fprintf(fid, '! Restarted from: %s\n', filename);
fprintf(fid, '! Generated by G16_restart (G16 Toolbox / MATLAB)\n');
fprintf(fid, '!\n');
