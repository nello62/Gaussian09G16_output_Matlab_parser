function T = G16_available_data(filename)
% G16_AVAILABLE_DATA  Lists which toolbox quantities/functions are
%                     expected to be extractable from a Gaussian 16
...
%   (ZPE/H/G/S), so calling G16_nmodes/G16_spectra on it raises an error
%   and G16_energy's *_corr/E0/U/H/G/T/P/S fields come back NaN -- none
%   of that is a bug, it is simply not in the file. This function lets
%   you check that in one call before running the extraction functions,
%   rather than discovering it one error/NaN at a time.
%
...
%
%   See also G16_ROUTE, G16_READ_ALL.
%
...
has_field       = any(cellfun(@(t) startsWith(t, 'field='), tokens));
...
add('hyperpolar (vibrational Beta)',   has_polar && has_freq, 'polar + freq',             'G16 only');
add('hyperpolar (electronic Beta)',    has_polar && has_field && has_cphf_rdfreq, 'polar + field=... + cphf=rdfreq', 'G16 only');
add('tddft',                           has_td,         'td',                             'G16 only');

T = table(Function(:), logical(Available(:)), Requires(:), Notes(:), ...
    'VariableNames', {'Function', 'Available', 'Requires', 'Notes'});

fprintf('\n── G16_available_data: %s ──\n', filename);
fprintf('  Route: %s\n\n', route);
