...
% arrow is drawn. G16_dipole_polar's own console summary is suppressed
...
try
    if show_dipole
        ...
        [~, dp] = evalc('G16_dipole_polar(filename, ''units'', ''Debye'', ''Lines'', lines)');
    end
...
if do_plot
    ...
    for i = 1:numel(q_use)
        ...
        text(ax, xyz_use(i,1), xyz_use(i,2), xyz_use(i,3) + r_off, ...
             ...
             'Interpreter', 'none', 'HandleVisibility', 'off','interpreter','latex');
    end
    rotate3d(ax, 'on');
