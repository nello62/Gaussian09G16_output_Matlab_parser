function ch_out = G16_charges_fchk(mol, ch, varargin)
% G16_CHARGES_FCHK  Visualises atomic charges from a G16_fchk_read struct
%                   directly, without requiring a .log / .out file.
%
%   ch_out = G16_CHARGES_FCHK(mol, ch)
%   ch_out = G16_CHARGES_FCHK(mol, ch, Name, Value, ...)
%
%   Designed to work with the output of G16_fchk_read:
%
%       data   = G16_fchk_read('3typ.fchk');
%       ch_out = G16_charges_fchk(data.mol, data.ch);
%
%   The function mirrors the interface of G16_charges exactly, so the two
...
%
%       ch    struct   charge struct from G16_fchk_read (data.ch)
...
%                               use G16_charges on the .log file for that)
...
%
%   OUTPUT:
%       ch_out    struct with fields (same as G16_charges output):
...
%
%       % Use as drop-in replacement for G16_charges:
...
for k = 1:numel(required_mol)
    if ~isfield(mol, required_mol{k})
        error('G16_charges_fchk: mol is missing field "%s". Use data.mol from G16_fchk_read.', ...
            required_mol{k});
    end
end
...
for k = 1:numel(required_ch)
    if ~isfield(ch, required_ch{k})
        error('G16_charges_fchk: ch is missing field "%s". Use data.ch from G16_fchk_read.', ...
            required_ch{k});
    end
end

if mol.Natoms ~= ch.Natoms
    error('G16_charges_fchk: mol.Natoms (%d) ≠ ch.Natoms (%d).', ...
...
switch mode
    ...
    case 'heavy'
        if isempty(q_heavy)
            warning(['G16_charges_fchk: H-summed charges (charges_H) are not available ' ...
                'in .fchk files. Falling back to per-atom charges.\n' ...
                'For H-summed charges, use G16_charges on the .log file.']);
            syms_use = mol.symbols;
            xyz_use  = mol.xyz;
            q_use    = q_atom;
        else
    ...
    otherwise
        error('G16_charges_fchk: mode must be ''atom'' or ''heavy''.');
end

% -------------------------------------------------------------------------
% Build output struct  (identical layout to G16_charges)
% -------------------------------------------------------------------------
src = '';
