...
if ~any(isnan(force_const(:))) && numel(masses) == Nat
    ...
    % dip_deriv is already (3 × N3): row = dipole Cartesian component
    % (x/y/z), column = displacement direction k (1..3Nat) -- built above
    % via reshape(dip_deriv_raw, 3, N3), which is already the correct
    % storage. No further reshape is needed (and reshaping an
    % already-(3,N3) matrix to (N3,3) and transposing back does NOT
    % undo itself -- it reinterprets the same column-major buffer under
    % a different shape, scrambling which values belong to which
    % displacement/component. This previously produced silently wrong
    % IR/Raman intensities, caught by comparing against the same
    % molecule's IR/Raman as printed directly by Gaussian in the
    ...
    if ~any(isnan(dip_deriv(:)))
        DD     = dip_deriv;                    % (3 × N3): already correct storage
        dmu_dQ = DD * L_vib_mn;                % (3 × Nmodes_vib)
        IR_int = sum(dmu_dQ.^2, 1)' * C_IR;
    end

    % ── Raman activities (Å⁴/AMU) ────────────────────────────────────────
    % pol_deriv is already (6 × N3): row = alpha component
    % (xx,xy,yy,xz,yz,zz), column = displacement direction -- see the
    % IR intensities note above for why no further reshape is applied.
    ...
    if ~any(isnan(pol_deriv(:)))
        PD     = pol_deriv;                    % (6 × N3): already correct storage
