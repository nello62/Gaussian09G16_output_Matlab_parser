function G16_draw_orbital(oe, varargin)
% G16_DRAW_ORBITAL  Draws a molecular-orbital energy-level diagram from
%                    the struct returned by G16_ORBITAL_ENERGIES,
%                    highlighting the HOMO-LUMO transition with an arrow
%                    labelled with the gap.
%
%   G16_draw_orbital(oe)
%   G16_draw_orbital(oe, Name, Value, ...)
%
%   Input:
%       oe  - struct returned by G16_orbital_energies
%
...
%
%   Example:
%       oe = G16_orbital_energies('a1.out');
%       G16_draw_orbital(oe)
%       G16_draw_orbital(oe, 'NLevels', 8, 'Units', 'Hartree')
%
%   See also G16_ORBITAL_ENERGIES.
...
if ~isstruct(oe) || ~isfield(oe, 'alpha_occ') || ~isfield(oe, 'alpha_virt')
    error('G16_draw_orbital: oe must be the struct returned by G16_orbital_energies.');
end
if isempty(oe.alpha_occ) || isempty(oe.alpha_virt)
    error('G16_draw_orbital: oe has no occupied/virtual orbital energies to plot.');
end
...
switch units
    ...
    case 'hartree'
        ...
        unit_lbl = 'Ha';
    otherwise
        error('G16_draw_orbital: ''Units'' must be ''eV'' or ''Hartree''.');
end
