...
% -------------------------------------------------------------------------
    ...
    % G16_animate_mode, using the Scale/AtomScale/BondTol/ShowLabels/
    ...
    % no mode figure is open or its view cannot be read.
        ...
        try
            G16_animate_mode(mol, nm, k, ...
                ...
                'View',       viewAngle);
            fprintf('G16_modeViewer: animation saved to %s\n', outFile);
        catch ME
            uialert(selFig, ME.message, 'G16_animate_mode error');
        end
    end
