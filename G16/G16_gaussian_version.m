function gv = G16_gaussian_version(filename)
% G16_GAUSSIAN_VERSION  Detects which Gaussian version/revision produced a
%                       .out/.log/.fchk file.
%
...
%
%   Example:
%       gv = G16_gaussian_version('a1.out');       % gv.major = 16, gv.revision = 'C.01'
%       gv = G16_gaussian_version('molecule.fchk'); % falls back to molecule.out/.log if present

if ~isfile(filename)
    error('G16_gaussian_version: file not found: %s', filename);
end
...
if isempty(major)
    warning('G16_gaussian_version:notFound', ...
