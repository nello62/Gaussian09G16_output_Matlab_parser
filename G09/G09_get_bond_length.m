function bondTable = G09_get_bond_length(mol, varargin)
%G09_GET_BOND_LENGTH  Costruisce la tabella delle distanze di legame di una molecola.
%
%   bondTable = G09_GET_BOND_LENGTH(mol) analizza la struttura MOL (come
%   quelle prodotte dal toolbox G09/G16, con campi .symbols [1xN cell o
...
%
%   bondTable = G09_GET_BOND_LENGTH(mol, 'Name', Value, ...) consente di
...
%   Esempio:
%     load('M.mat');
%     T = G09_get_bond_length(molzn, 'SortBy', 'atom');
%     T = G09_get_bond_length(mol3tpy, 'IncludeH', false, ...
...
%   connettività "ufficiale" dell'output G09/G16.
...
if size(xyz,1) ~= Natoms
    error('G09_get_bond_length:sizeMismatch', ...
...
end % function G09_get_bond_length
...
if isKey(R, sym)
    ...
    warning('G09_get_bond_length:unknownElement', ...
