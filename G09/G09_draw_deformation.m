function G09_draw_deformation(mol1, mol2, varargin)
% G09_DRAW_DEFORMATION  Visualises the geometric deformation between two
...
%               G09_structure, G_read_structure_file, G09_fchk_read's
...
%   G09_DRAW_MODE for a normal mode, but here the actual geometric shift
...
%       'ArrowColor'     - (default [1.0 0.4 0.1], same as G09_draw_mode)
...
%
%   Example:
%       mol1 = G09_structure('nofield.out');
%       mol2 = G09_structure('field_x025.out');
%       G09_draw_deformation(mol1, mol2, 'Scale', 200);
%       G09_draw_deformation(mol1, mol2, 'Overlay', true, 'Scale', 200);
%
%   See also G09_DRAW_MOLECULE, G09_DRAW_MODE, G_MATCH_MODES.
%
...
if ~isstruct(mol1) || ~isfield(mol1, 'xyz') || ~isstruct(mol2) || ~isfield(mol2, 'xyz')
    error('G09_draw_deformation: mol1 and mol2 must both be mol structs with .xyz.');
end
if mol1.Natoms ~= mol2.Natoms
    error('G09_draw_deformation: mol1.Natoms (%d) does not match mol2.Natoms (%d) -- not the same molecule.', ...
...
if max_d == 0
    warning('G09_draw_deformation: mol1 and mol2 have identical coordinates -- nothing to draw.');
    return
end
...
G09_draw_molecule(mol1, 'Ax', ax, 'AtomScale', atom_scale, 'BondTol', bond_tol, ...
...
if do_overlay
    bondTable2 = G09_get_bond_length(mol2, 'Tolerance', bond_tol, 'IncludeH', true);
...
end % G09_draw_deformation


% =========================================================================
%  3D arrow with cone tip (same construction as G09_draw_mode's own
