function outfile = G09_write_report(T, outfile)
% G09_WRITE_REPORT  Writes a human-readable text report from a G09_read_all struct.
%
%   G09_WRITE_REPORT(T) writes a formatted summary of every field in T
%   (the struct returned by G09_READ_ALL) to a .txt file, named after the
%   source Gaussian file (e.g. 'zeatin.log' -> 'zeatin_report.txt') in the
%   current folder.
%
%   G09_WRITE_REPORT(T, OUTFILE) writes to OUTFILE instead.
%
%   OUTFILE = G09_WRITE_REPORT(...) also returns the path written to.
%
...
%
%   Example:
%       T = G09_read_all('zeatin.log');
%       G09_write_report(T);
%       G09_write_report(T, 'zeatin_summary.txt');
%
%   See also G09_READ_ALL.
...
fid = fopen(outfile, 'w');
if fid == -1
    error('G09_write_report:cannotOpen', 'Could not open %s for writing.', outfile);
end
cleaner = onCleanup(@() fclose(fid));

fprintf(fid, '================================================================\n');
fprintf(fid, '  Gaussian 09 Calculation Report\n');
fprintf(fid, '================================================================\n');
if isfield(T, 'structure') && isfield(T.structure, 'filename')
...
if isfield(T, 'energy')
    ...
    if e.has_thermo
        ...
        if isfield(e, 'U')
            ...
            u_val = e.SCF + e.U_corr;   % G09_energy has no .U field; compute it
        end
...
fprintf('G09_write_report: report written to %s\n', outfile);

end % G09_write_report
