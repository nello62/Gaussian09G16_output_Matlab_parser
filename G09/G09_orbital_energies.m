function oe = G09_orbital_energies(filename, varargin)
% G09_ORBITAL_ENERGIES  Extracts molecular orbital energies (HOMO/LUMO and
...
%   selects which block to report on, exactly like G09_ENERGY/G09_STRUCTURE.
%
...
if isempty(blocks)
    error('G09_orbital_energies: no orbital eigenvalue block found in %s', filename);
end
...
if ischar(step_req)
    ...
    else, error('G09_orbital_energies: step must be ''first'', ''last'', or an integer.');
    end
else
    si = round(step_req);
    if si < 1 || si > Nsteps
        error('G09_orbital_energies: step %d out of range [1, %d].', si, Nsteps);
    end
end

blk = blocks(si);
if isempty(blk.alpha_occ) || isempty(blk.alpha_virt)
    error('G09_orbital_energies: incomplete eigenvalue block (step %d) in %s', si, filename);
end
...
% Print summary
% -------------------------------------------------------------------------
fprintf('\n── G09_orbital_energies (step %d/%d): %s ──\n', si, Nsteps, filename);
