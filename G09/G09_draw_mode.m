function G09_draw_mode(mol, nm, mode_idx, varargin)
% G09_DRAW_MODE  Visualises a normal mode on a 3D molecular structure.
%
%   G09_draw_mode(mol, nm, mode_idx)
%   G09_draw_mode(mol, nm, mode_idx, Name, Value, ...)
%
%   Input:
%       mol       - struct returned by G09_structure (geometry)
%       nm        - struct returned by G09_nmodes (displacement vectors)
...
if mode_idx < 1 || mode_idx > nm.Nmodes
    error('G09_draw_mode: mode index %d is out of range [1, %d]', mode_idx, nm.Nmodes);
end
if mol.Natoms ~= nm.Natoms
    error('G09_draw_mode: mol.Natoms (%d) does not match nm.Natoms (%d)', mol.Natoms, nm.Natoms);
end
...
if max_norm > 0
    ...
    warning('G09_draw_mode: zero displacement vectors for mode %d', mode_idx);
    return
end
