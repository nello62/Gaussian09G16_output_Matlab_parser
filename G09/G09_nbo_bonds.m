function bt = G09_nbo_bonds(filename, varargin)
% G09_NBO_BONDS  Determines bond order (single/double/triple) from a
%                Gaussian NBO (Natural Bond Orbital) analysis.
%
%   bt = G09_NBO_BONDS(filename)
%   bt = G09_NBO_BONDS(filename, 'section', 'last')   % default
%   bt = G09_NBO_BONDS(filename, 'section', 'first')
%
...
%
%   Unlike G09_GET_BOND_LENGTH (a purely geometric covalent-radius
%   criterion) and G09_DRAW_MOLECULE's bond-order heuristic (a bond
...
if isempty(lines)
    if ~isfile(filename)
        error('G09_nbo_bonds: file not found: %s', filename);
    end
